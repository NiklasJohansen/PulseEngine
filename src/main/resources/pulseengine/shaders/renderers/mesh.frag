...
{
    if (aoMetalRoughTex.x <= 0.0) return vec3(1.0, 0.0, 1.0); // no ao, non-metal, fully rough
...
void main()
{
    ...
    float roughness    = clamp(aomr.g, 0.2, 1.0);
    ...
    roughness = clamp(sqrt(roughness * roughness + toksvig * toksvig + geom * geom), 0.04, 1.0);
    ...
    F0 = mix(F0, specularColor.rgb, clamp(specularColor.a * (1.0 - metallic), 0.0, 1.0));
