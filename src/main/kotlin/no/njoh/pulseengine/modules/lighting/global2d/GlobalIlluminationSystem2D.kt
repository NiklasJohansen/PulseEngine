...
import no.njoh.pulseengine.modules.lighting.global2d.effects.*
import no.njoh.pulseengine.modules.lighting.global2d.effects.GiJfa.JfaMode.*
import no.njoh.pulseengine.modules.lighting.global2d.effects.GiRadianceCascades.Companion.BASE_RAY_COUNT
...
open class GlobalIlluminationSystem2D : SceneSystem()
{
    ...
    private val normalMapRenderPass   = EntityRenderPass<NormalMapped2D>(GI_NORMAL_MAP)  { e, surface -> onRenderNormalMap(e, surface)   }
    private val localOccluderPass     = EntityRenderPass<GiOccluder2D>(GI_LOCAL_SCENE)     { e, surface -> onRenderOccluder(e, surface)    }
    private val localLightSourcePass  = EntityRenderPass<GiLightSource2D>(GI_LOCAL_SCENE)  { e, surface -> onRenderLightSource(e, surface) }
    private val globalOccluderPass    = EntityRenderPass<GiOccluder2D>(GI_GLOBAL_SCENE)    { e, surface -> onRenderOccluder(e, surface)    }
    private val globalLightSourcePass = EntityRenderPass<GiLightSource2D>(GI_GLOBAL_SCENE) { e, surface -> onRenderLightSource(e, surface) }
