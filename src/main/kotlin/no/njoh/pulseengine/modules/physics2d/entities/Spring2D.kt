...
open class Spring2D : Common2DSceneEntity(), PhysicsEntity2D
{
    ...
    {
        val body0 = engine.scene.getEntityOfType<PhysicsBody2D>(fromBodyId)
        val body1 = engine.scene.getEntityOfType<PhysicsBody2D>(toBodyId)
        ...
        body0?.shape?.getPoint(fromBodyPointIndex)?.let { p0 ->
            ...
            body1?.shape?.getPoint(toBodyPointIndex)?.let { p1 ->
                ...
                val ratio0 = if (body1.bodyType == BodyType2D.STATIC) 1f else mass1 * invTotalMass
                val ratio1 = if (body0.bodyType == BodyType2D.STATIC) 1f else mass0 * invTotalMass

                if (body0.bodyType != BodyType2D.STATIC)
                {
                    body0.shape.setPoint(fromBodyPointIndex, p0x + xDisp * ratio0, p0y + yDisp * ratio0)
                    body0.wakeUp()
                }

                if (body1.bodyType != BodyType2D.STATIC)
                {
