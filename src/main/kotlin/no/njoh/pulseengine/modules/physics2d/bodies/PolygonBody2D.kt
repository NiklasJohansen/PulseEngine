...
import no.njoh.pulseengine.modules.physics2d.ContactSolver2D
import no.njoh.pulseengine.modules.physics2d.BodyType2D.STATIC
import no.njoh.pulseengine.modules.physics2d.ContactResult
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.LENGTH
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.N_POINT_FIELDS
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.POINT_0
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.POINT_1
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.RESTING_MIN_VEL
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.RESTING_STEPS_BEFORE_SLEEP
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.STIFFNESS
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.X
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.X_ACC
...
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.Y_ACC
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.Y_LAST
import kotlin.math.*

interface PolygonBody2D : PhysicsBody2D
{
    override val shape: PolygonShape2D
    ...
    private fun updateCollisions(engine: PulseEngine)
    {
        ...
        engine.scene.forEach2DEntityNearby(shape.xCenter, shape.yCenter, xMax - xMin, yMax - yMin)
        {
            if (it !== this &&
                it is PhysicsBody2D &&
                ...
                ContactSolver2D.solve(this, it)?.let { result ->
    ...
    fun onBodyUpdated()
    {
        if (this is SceneEntity && this is Spatial2D)
        {
            x = shape.xCenter
            y = shape.yCenter
    ...
    override fun onCollision(engine: PulseEngine, otherBody: PhysicsBody2D, result: ContactResult) { }
}