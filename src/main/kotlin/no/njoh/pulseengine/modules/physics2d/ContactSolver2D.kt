...
import no.njoh.pulseengine.modules.physics2d.BodyType2D.DYNAMIC
import no.njoh.pulseengine.modules.physics2d.BodyType2D.STATIC
import no.njoh.pulseengine.modules.physics2d.bodies.*
import no.njoh.pulseengine.modules.physics2d.shapes.CircleShape2D
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.N_POINT_FIELDS
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.X
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.X_LAST
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.Y
import no.njoh.pulseengine.modules.physics2d.shapes.PolygonShape2D.Companion.Y_LAST
import no.njoh.pulseengine.core.shared.utils.Logger
import no.njoh.pulseengine.core.shared.utils.MathUtil
import no.njoh.pulseengine.core.shared.utils.MathUtil.getLineSegmentCircleIntersection
import no.njoh.pulseengine.core.shared.utils.MathUtil.getLineSegmentIntersection
import no.njoh.pulseengine.core.shared.utils.MathUtil.pointToLineDistanceSquared
import no.njoh.pulseengine.core.shared.utils.MathUtil.pointToLineSegmentDistanceSquared
import org.joml.Vector2f
import kotlin.math.*

object ContactSolver2D
{
    private val result = ContactResult()
    private val impulse = Vector2f(0f, 0f)

    fun solve(b0: PhysicsBody2D, b1: PhysicsBody2D): ContactResult?
    {
        when (b0)
        {
            is PolygonBody2D -> when (b1)
            {
                is PolygonBody2D -> return solvePolygonOnPolygon(b0, b1)
                is PointBody2D   -> return solvePointOnPolygon(b1, b0)
                is CircleBody2D  -> return solveCircleOnPolygon(b1, b0)
                is LineBody2D    -> return null
            }
            is PointBody2D -> when (b1)
            {
                is PointBody2D   -> return null
                is PolygonBody2D -> return solvePointOnPolygon(b0, b1)
                is CircleBody2D  -> return solveCircleOnPoint(b1, b0)
                is LineBody2D    -> return null
            }
            is CircleBody2D -> when (b1)
            {
                is PolygonBody2D -> return solveCircleOnPolygon(b0, b1)
                is PointBody2D   -> return solveCircleOnPoint(b0, b1)
                is CircleBody2D  -> return solveCircleOnCircle(b0, b1)
                is LineBody2D    -> return solveCircleOnLine(b0, b1)
            }
            is LineBody2D -> when (b1)
            {
                is PolygonBody2D -> return null
                is PointBody2D   -> return null
                is CircleBody2D  -> return solveCircleOnLine(b1, b0)
                is LineBody2D    -> return null
            }
        }
    ...
    private fun solvePointOnPolygon(pointBody: PointBody2D, polygonBody: PolygonBody2D): ContactResult?
    {
    ...
    private fun solvePolygonOnPolygon(aBody: PolygonBody2D, bBody: PolygonBody2D): ContactResult?
    {
        var depth = Float.MAX_VALUE
        var xNormal = 0f
        var yNormal = 0f

        var edgeBody: PolygonBody2D? = null
        var edgePoint0 = -1
        var edgePoint1 = -1
    ...
    private fun solveCircleOnCircle(aBody: CircleBody2D, bBody: CircleBody2D): ContactResult?
    {
    ...
    private fun CircleShape2D.update(xNormal: Float, yNormal: Float, depth: Float, impulse: Float, coefficientOfFriction: Float, ratio: Float)
    {
        var xVel = x - xLast
        var yVel = y - yLast
    ...
    private fun solveCircleOnPolygon(circleBody: CircleBody2D, polygonBody: PolygonBody2D): ContactResult?
    {
    ...
    private fun solveCircleOnLine(circleBody: CircleBody2D, lineBody: LineBody2D): ContactResult?
    {
        val circle = circleBody.shape
        val xCircle = circle.x
        val yCircle = circle.y
        ...
        // Find the closest point along line within radius of circle
        val line = lineBody.shape
        val (x0, y0) = line.getPoint(0)
        val (x1, y1) = line.getPoint(1)
        val closestPoint = MathUtil.closestPointOnLineSegment(xCircle, yCircle, x0, y0, x1, y1)
        val xd = xCircle - closestPoint.x
        val yd = yCircle - closestPoint.y
        val distSquared = xd * xd + yd * yd
        if (distSquared > radius * radius)
            return null // No collision

        // Calculate collision normal and penetration depth
        val xDelta = closestPoint.x - xCircle
        val yDelta = closestPoint.y - yCircle
        val dist = sqrt(xDelta * xDelta + yDelta * yDelta)
        val xNormal = xDelta / dist
        val yNormal = yDelta / dist
        val depth = dist - radius
        ...
        // Calculate mass ratios, friction restitution coefficients
        val invTotalMass = 1.0f / (line.mass + circle.mass)
        val circleBodyRatio = if (lineBody.bodyType == STATIC) 1f else line.mass * invTotalMass
        val rigidBodyRatio = if (circleBody.bodyType == STATIC) 1f else circle.mass * invTotalMass
        val coefficientOfFriction = (lineBody.friction + circleBody.friction) * 0.5f
        val coefficientOfRestitution = (lineBody.restitution + circleBody.restitution) * 0.5f

        // Calculate where on the edge the collision point lies (0.0 - 1.0). If-check prevents divide by zero.
        val t = if (abs(x0 - x1) > abs(y0 - y1)) (closestPoint.x - x0) / (x1 - x0) else (closestPoint.y - y0) / (y1 - y0)

        if (lineBody.bodyType != STATIC)
        {
            // Calculate a scaling factor that ensures that the point lies on the edge after the collision response
            val lambda = 1.0f / (t * t + (1 - t) * (1 - t)) * rigidBodyRatio

            // Correct edge point positions
            line.x0 -= xNormal * depth * lambda * (1 - t)
            line.y0 -= yNormal * depth * lambda * (1 - t)
            line.x1 -= xNormal * depth * lambda * t
            line.y1 -= yNormal * depth * lambda * t

            // Add friction to point 0
            val xVel0 = x0 - line.xLast0
            val yVel0 = y0 - line.yLast0
            val frictionImpulse0 = calculateFrictionImpulse(xVel0, yVel0, yNormal, -xNormal, depth, coefficientOfFriction)
            line.xLast0 += frictionImpulse0.x * (1 - t)
            line.yLast0 += frictionImpulse0.y * (1 - t)

            // Add friction to point 1
            val xVel1 = x1 - line.xLast1
            val yVel1 = y1 - line.yLast1
            val frictionImpulse1 = calculateFrictionImpulse(xVel1, yVel1, yNormal, -xNormal, depth, coefficientOfFriction)
            line.xLast1 += frictionImpulse1.x * t
            line.yLast1 += frictionImpulse1.y * t
        }
        ...
        if (circleBody.bodyType != STATIC && dot > 0f)
        {
            ...
            if (lineBody.bodyType != STATIC)
            {
                line.xLast0 += xVelChange * (1f - t)
                line.yLast0 += yVelChange * (1f - t)
                line.xLast1 += xVelChange * t
                line.yLast1 += yVelChange * t
            }
    ...
    private fun solveCircleOnPoint(circleBody: CircleBody2D, pointBody: PointBody2D): ContactResult?
    {
...
private operator fun Vector2f.component1() = this.x
private operator fun Vector2f.component2() = this.y
